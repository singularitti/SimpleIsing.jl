...
!!! info
    The style guide is always a work in progress, and not all SimpleIsing code
    follows the rules. When modifying SimpleIsing, please fix the style violations
...
`SimpleIsing` uses [`JuliaFormatter`](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/SimpleIsing.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the `SimpleIsing` directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to `SimpleIsing` have
